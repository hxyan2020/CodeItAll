/**
 * CodeItAll HTML Lab — markup editor + live iframe preview
 * Separate track from Python / C++ / JS Labs.
 */
const state = {
  ready: true,
  busy: false,
...
function openProject(id) {
  ...
  $("#projectDesc").textContent =
    project.description || "Edit HTML, preview live, ask AI when stuck.";
  ...
  project.cells.forEach((cell, idx) => {
    ...
    wrap.innerHTML = `
      ...
      <div class="cell-toolbar">
        ...
        <div class="cell-actions">
          <button type="button" class="btn ok run-btn">Preview</button>
    ...
    if (window.CodeMirror) {
      ...
      editor.setSize("100%", Math.max(200, Math.min(480, (cell.source || "").split("\n").length * 18 + 48)));
      state.editors[idx] = editor;
      window.LabExplain?.attach?.(editor, "html");
    }
  ...
  requestAnimationFrame(() => {
    ...
    if (state.cellEls.length) runCell(0);
  });
}
...
function wrapFragment(src) {
  ...
  if (/<!DOCTYPE/i.test(trimmed) || /<html[\s>]/i.test(trimmed)) return src;
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8" />
...
<title>Preview</title>
<style>body{font-family:Georgia,serif;margin:1.25rem;line-height:1.5;}</style>
</head>
<body>
${src}
</body>
...
async function runCell(idx) {
  ...
  label.textContent = `Step ${idx + 1} · preview`;
  setStatus("Rendering…", "loading");

  try {
    if (!src.trim()) {
      appendOutput(idx, "Add some HTML, then click Preview.", true);
      setStatus("Empty", "error");
      return;
    }
    const iframe = document.createElement("iframe");
    iframe.title = `Preview step ${idx + 1}`;
    iframe.setAttribute("sandbox", "allow-forms allow-scripts allow-same-origin allow-popups allow-modals");
    iframe.srcdoc = wrapFragment(src);
    appendOutput(idx, iframe);
    const note = document.createElement("div");
    note.className = "ok";
    note.style.marginTop = "0.35rem";
    note.style.fontSize = "0.8rem";
    note.style.opacity = "0.7";
    note.textContent = "Live preview (sandboxed iframe)";
    appendOutput(idx, note);
...
function maybeOfferAi(idx) {
  ...
  el.innerHTML = `
    ...
    Step ${idx + 1} had a problem. Tap <strong>Ask AI</strong> for a fix.
  `;
  box.appendChild(el);
  box.scrollTop = box.scrollHeight;
}
...
function getSettings() {
  return {
    endpoint: localStorage.getItem("cia_html_ai_endpoint") || localStorage.getItem("cia_ai_endpoint") || "/api/assist",
    apiKey: localStorage.getItem("cia_html_ai_key") || localStorage.getItem("cia_ai_key") || "",
    model: localStorage.getItem("cia_html_ai_model") || localStorage.getItem("cia_ai_model") || "gpt-4o-mini",
  };
}

function saveSettingsFromForm() {
  localStorage.setItem("cia_html_ai_endpoint", $("#aiEndpoint").value.trim() || "/api/assist");
  localStorage.setItem("cia_html_ai_key", $("#aiKey").value.trim());
  localStorage.setItem("cia_html_ai_model", $("#aiModel").value.trim() || "gpt-4o-mini");
...
async function askAiForCell(idx) {
  ...
  const question = err
    ? `This HTML step failed to preview. Explain and give a corrected full HTML document.\n\nError:\n${err}\n\nCode:\n${code}`
    : `Review this HTML step from "${project?.title || "project"}". Suggest semantic improvements for a learner.\n\nCode:\n${code}`;
...
async function sendAi(prompt, context = {}) {
  ...
  try {
    ...
    const match = reply.match(/```(?:html|html5)?\n([\s\S]*?)```/i);
    if (match && context.cellIndex != null) {
      ...
      apply.onclick = () => {
        ...
        appendToast(`Applied AI suggestion to step ${context.cellIndex + 1}. Preview to verify.`);
        runCell(context.cellIndex);
      };
        ...
        messages: [
          {
            role: "system",
            content:
              "You are CodeItAll HTML Lab AI Assist. Help beginners learn HTML. Be concise. Prefer semantic HTML5. When fixing markup, return a full document inside a ```html fence.",
          },
          {
            role: "user",
            content: [
...
function localTutor(payload) {
  ...
  if (code && !/<!DOCTYPE/i.test(code) && !/<html/i.test(code)) {
    tips.push("Tip: Start with <!DOCTYPE html> and wrap content in <html>, <head>, <body>.");
  }
  if (/<img(?![^>]*alt=)/i.test(code)) {
    tips.push("An <img> may be missing alt text — add alt for accessibility.");
  }
  if (/<input(?![^>]*id=)/i.test(code) && /<label/i.test(code) === false) {
    tips.push("Wire labels with for/id so clicks focus the right input.");
  }
  if (!code.trim()) {
    tips.push("Paste some HTML, Preview, then Ask AI again.");
  }
  return tips.join("\n\n");
}
...
async function main() {
  ...
  try {
    ...
    $("#runCellHint").textContent = "Failed to load HTML projects.json.";
    console.error(err);
  }
}
