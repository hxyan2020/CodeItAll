/**
 * CodeItAll C++ Lab — step editor + remote g++ via /api/cpp/compile
 * Separate track from the Python Lab (Pyodide).
 */
const COMPILE_ENDPOINT = "/api/cpp/compile";
...
function openProject(id) {
  ...
  $("#projectDesc").textContent =
    project.description || "Edit steps, compile with g++, ask AI when stuck.";
  ...
  project.cells.forEach((cell, idx) => {
    ...
    if (window.CodeMirror) {
      ...
      window.LabExplain?.attach?.(editor, "cpp");
    }
...
async function compileCpp(code) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Compile API ${res.status}: ${t.slice(0, 300)}`);
  }
  return res.json();
}
...
async function runCell(idx) {
  if (state.busy) {
    appendOutput(idx, "Already compiling another step — wait a moment.", true);
    return;
  }
  state.busy = true;
  clearOutput(idx);
  ...
  label.textContent = `Step ${idx + 1} · compiling…`;
  setStatus("Compiling…", "loading");

  try {
    const data = await compileCpp(src);
    const parts = [];
    if (data.compiler_output) parts.push(data.compiler_output.trim());
    if (data.compiler_error) {
      appendOutput(idx, [data.compiler_error, ...parts].filter(Boolean).join("\n\n"), true);
      label.textContent = `Step ${idx + 1} · compile error`;
      setStatus("Compile error", "error");
      maybeOfferAi(idx);
      return;
    }
    if (data.program_error) parts.push(data.program_error.trim());
    if (data.signal) parts.push(`Signal: ${data.signal}`);
    const out = (data.program_output || "").trimEnd();
    if (out) parts.unshift(out);
    if (parts.length) appendOutput(idx, parts.join("\n\n"), !data.ok && String(data.status) !== "0");
    else appendOutput(idx, "✓ Program finished (no printed output).");
    label.textContent = `Step ${idx + 1}`;
    setStatus("g++ ready", "ready");
  } catch (err) {
...
function getSettings() {
  return {
    endpoint: localStorage.getItem("cia_cpp_ai_endpoint") || localStorage.getItem("cia_ai_endpoint") || "/api/assist",
    apiKey: localStorage.getItem("cia_cpp_ai_key") || localStorage.getItem("cia_ai_key") || "",
    model: localStorage.getItem("cia_cpp_ai_model") || localStorage.getItem("cia_ai_model") || "gpt-4o-mini",
  };
}

function saveSettingsFromForm() {
  localStorage.setItem("cia_cpp_ai_endpoint", $("#aiEndpoint").value.trim() || "/api/assist");
  localStorage.setItem("cia_cpp_ai_key", $("#aiKey").value.trim());
  localStorage.setItem("cia_cpp_ai_model", $("#aiModel").value.trim() || "gpt-4o-mini");
...
async function askAiForCell(idx) {
  ...
  const question = err
    ? `This C++ step failed to compile or run. Explain the error and give a corrected full program.\n\nError:\n${err}\n\nCode:\n${code}`
    : `Review this C++ step from "${project?.title || "project"}". Suggest improvements or explain what it does for a learner.\n\nCode:\n${code}`;
...
async function sendAi(prompt, context = {}) {
  ...
  try {
    ...
    const match = reply.match(/```(?:cpp|c\+\+|c)?\n([\s\S]*?)```/i);
        ...
        messages: [
          {
            role: "system",
            content:
              "You are CodeItAll C++ Lab AI Assist. Help beginners learn C++. Be concise. When fixing code, return a full corrected program inside a ```cpp fence.",
          },
          {
            role: "user",
            content: [
...
function localTutor(payload) {
  ...
  if (/error:/i.test(err)) {
    tips.push("Start with the first compiler error line — later errors are often cascading.");
  }
  if (/expected|semicolon/i.test(err)) {
    tips.push("Missing semicolon, brace, or return type is a common beginner mistake.");
  }
  if (/undefined reference|was not declared/i.test(err)) {
    tips.push("Check includes (<iostream>, <vector>, …) and that names match declared functions.");
  }
  if (/segmentation|signal/i.test(err)) {
    tips.push("Likely out-of-bounds index or null pointer. Print sizes before accessing containers.");
  }
  if (!err && payload.code) {
    tips.push("Looks runnable — click Run. Prefer g++ -std=c++17 and STL containers.");
  }
  return tips.join("\n\n");
}
...
async function main() {
  ...
  try {
    ...
    $("#runCellHint").textContent = "Failed to load C++ projects.json.";
    console.error(err);
  }
}
